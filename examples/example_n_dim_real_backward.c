 ...
 *  AOCL-FFTZ APIs to compute ND real backward FFT problem for FFTZ_DOUBLE
 * datatype on ILP64 systems. Helper macros and functions used for
 * initialization of the problem descriptor are provided in helpers.h.
 *
...
int main()
{
    ...
    aoclfftz_prob_desc_d_64_ problem = {
        ...
        .pthr_fft =
            {/*
              ...
              * Library is built with ENABLE_MULTI_THREADING=ON for
     ...
     */
    FFTZ_UINTP in_data_stride = 2;
    FFTZ_UINTP out_data_stride = 1;

    /* STEP 1.3: Allocate memory input/output buffers and
                 create new output buffer in case of out-of-place problem.
                 For in-place real FFT, use max(in_size,
       out_size*out_data_stride). */
    FFTZ_DOUBLE *in = NULL;
    FFTZ_DOUBLE *out = NULL;
    FFTZ_UINTP alloc_size = in_buffer_size * in_data_stride;
    if (!is_out_of_place && problem.flags.fft_type == 1)
    {
        FFTZ_UINTP out_doubles = out_buffer_size * out_data_stride;
        if (out_doubles > alloc_size)
        {
            alloc_size = out_doubles;
        }
    }
    ALLOC(in, FFTZ_DOUBLE, alloc_size);
    if (is_out_of_place)
    {
        ALLOC(out, FFTZ_DOUBLE, (out_buffer_size * out_data_stride));
    }

    /* STEP 1.4: Prepare input for FFT calculation and
                 Use input buffer as output for in-place buffer.
                 For real backward, input is complex so fill
       in_buffer_size*in_data_stride doubles. */
    FFTZ_UINTP in_elems_to_fill = (in_buffer_size * in_data_stride)
                             / (FFTZ_UINTP)DATA_STRIDE(problem.flags.fft_type);
    PREPARE_RANDOM_INPUT(in, in_elems_to_fill, problem.flags.fft_type,
                         FFTZ_DOUBLE);
    problem.in = in;
