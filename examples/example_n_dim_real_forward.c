 ...
 *  AOCL-FFTZ APIs to compute ND real forward FFT problem for FFTZ_DOUBLE
...
int main()
{
     ...
     */
    FFTZ_UINTP in_data_stride = 1;
    FFTZ_UINTP out_data_stride = 2;

    /* STEP 1.3: Allocate memory input/output buffers and
                 create new output buffer in case of out-of-place problem.
                 For in-place real FFT, output is half-complex (out_data_stride
       doubles per element), so allocate max(in_size, out_size*out_data_stride).
     */
    FFTZ_DOUBLE *in = NULL;
