...
/** @file twid_r2c_fft16c.c
 *
 *  @brief R2C fused twiddle (forward twiddle + conjugate output) Radix-16 FFT
...
static FFTZ_VOID twid_r2c_fft16c_fp64(FFTZ_VOID *in_real, FFTZ_VOID *in_imag,
...
static FFTZ_VOID twid_r2c_fft16c_fp32(FFTZ_VOID *in_real, FFTZ_VOID *in_imag,
                                      FFTZ_VOID *out_real, FFTZ_VOID *out_imag,
                                      FFTZ_INTP n, aoclfftz_strides_t *strides,
                                      FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    ...
    for (cnt = 0; cnt < n; cnt++)
    {
...

        {
...

            {
                // Point 0 has the trivial twiddle (1, 0); the linear buffer
                // drops the k = 0 row, so use the constant directly.
                const FFTZ_FLOAT twid_r1 = 1.0;
                const FFTZ_FLOAT twid_i1 = 0.0;
                                      ...
                                      FFTZ_UINT8 direction /* unused */)
{
    if (precision == DT_FLOAT)
    {
        return twid_r2c_fft16c_fp32;
    }
    else if (precision == DT_DOUBLE)
    {
        return twid_r2c_fft16c_fp64;
    }
    else
    {
        return NULL;
