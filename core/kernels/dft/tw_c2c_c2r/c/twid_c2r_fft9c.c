// Copyright Advanced Micro Devices, Inc.
// SPDX-License-Identifier: BSD-3-Clause

/** @file twid_c2r_fft9c.c
 *
 *  @brief C2R fused twiddle (conjugate input + twiddle output) Radix-9 FFT
 * kernel with scalar operations in C
 *
 *  This file contains the DIT twiddle radix-9 FFT implementations using scalar
 *  operations for single-precision and double-precision inputs.
 *
 *  @author Ashwin K. Godbole
 */

#include "core/kernels/kernel.h"

static const ops_cycles_t ops_cnt[NUM_PRECISIONS] = {{0, 72, 96, 52, 0, 0},
                                                     {0, 72, 96, 52, 0, 0}};

ops_cycles_t get_ops_cnt_twid_c2r_fft9c(UINT8 precision, UINT8 direction)
{
    if (precision == DT_FLOAT)
    {
        return ops_cnt[0];
    }
    else
    {
        return ops_cnt[1];
    }
}

static VOID twid_c2r_fft9c_fp64(VOID *in_real, VOID *in_imag, VOID *out_real,
                                VOID *out_imag, INTP n,
                                aoclfftz_strides_t *strides, VOID *twd,
                                UINT8 flag)
{
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Enter");
    const DOUBLE CRTM_9_1 = +0.939692620785908384054109277324731469936208134;
    const DOUBLE CRTM_9_2 = +0.342020143325668733044099614682259580763083368;
    const DOUBLE CRTM_9_3 = +0.984807753012208059366743024589523013670643252;
    const DOUBLE CRTM_9_4 = +0.173648177666930348851716626769314796000375677;
    const DOUBLE CRTM_9_5 = +0.642787609686539326322643409907263432907559884;
    const DOUBLE CRTM_9_6 = +0.766044443118978035202392650555416673935832457;
    const DOUBLE CRTM_9_7 = +0.500000000000000000000000000000000000000000000;
    const DOUBLE CRTM_9_8 = +0.866025403784438646763723170752936183471402627;

    DOUBLE *in_h1_r, *in_h2_r, *in_h1_i, *in_h2_i, *out_h1_r, *out_h2_r,
        *out_h1_i, *out_h2_i;
#ifdef VOLATILE_STRIDE_ARRAY
    volatile INTP *in_strides = strides->in_strides;
    volatile INTP *out_strides = strides->out_strides;
#else
    INTP *in_strides = strides->in_strides;
    INTP *out_strides = strides->out_strides;
#endif
    INTP v_in_stride = strides->v_in_stride;
    INTP v_in_h2_stride = strides->v_in_h2_stride;
    INTP v_out_stride = strides->v_out_stride;
    INTP v_out_h2_stride = strides->v_out_h2_stride;

    aoclfftz_twiddle_t *tws = (aoclfftz_twiddle_t *)twd;
    DOUBLE *tw = (DOUBLE *)(tws->TW);
    UINTP cols = tws->cols;
    UINTP load_multi_cols = tws->load_multi_cols;

    in_h1_r = (DOUBLE *)in_imag;
    in_h2_r = in_h1_r;
    in_h1_i = (DOUBLE *)in_real;
    in_h2_i = in_h1_i;
    out_h1_r = (DOUBLE *)out_imag;
    out_h2_r = out_h1_r;
    out_h1_i = (DOUBLE *)out_real;
    out_h2_i = out_h1_i;

    for (INTP cnt = 0; cnt < n; cnt++)
    {
        DOUBLE v1r, v1i, v2r, v2i, v3r, v3i, v4r, v4i, v5r, v5i, v6r, v6i, v7r,
            v7i, v8r, v8i, v9r, v9i;
        DOUBLE v1rr, v1ii, tv1rr, tv1ii, tv2rr, tv2ii, cv1rr, cv1ii, cv2rr,
            cv2ii, cv3rr, cv3ii, cv4rr, cv4ii, cv5rr, cv5ii, cv6rr, cv6ii,
            cv7rr, cv7ii, cv8rr, cv8ii, cv9rr, cv9ii;
        DOUBLE tv1rr1, tv1rr2, tv1rr3, tv1ii1, tv1ii2, tv1ii3;
        DOUBLE tv2rr1, tv2rr2, tv2rr3, tv2ii1, tv2ii2, tv2ii3;
        DOUBLE tv3rr, tv3ii, tv3ir, tv3ir1, tv4ir, tv4ir1;

        v1r = *in_h1_r;
        v1i = *in_h1_i;

        // Input point 2: x(1)
        DOUBLE v2r_t = in_h1_r[in_strides[1]];
        DOUBLE v2i_t = in_h1_i[in_strides[1]];
        v2r = v2r_t;
        v2i = v2i_t;

        // Input point 3: x(2)
        DOUBLE v3r_t = in_h1_r[in_strides[2]];
        DOUBLE v3i_t = in_h1_i[in_strides[2]];
        v3r = v3r_t;
        v3i = v3i_t;

        // Input point 4: x(3)
        DOUBLE v4r_t = in_h1_r[in_strides[3]];
        DOUBLE v4i_t = in_h1_i[in_strides[3]];
        v4r = v4r_t;
        v4i = v4i_t;

        // Input point 5: x(4)
        DOUBLE v5r_t = in_h1_r[in_strides[4]];
        DOUBLE v5i_t = in_h1_i[in_strides[4]];
        v5r = v5r_t;
        v5i = v5i_t;

        // Input point 6: x(5)
        DOUBLE v6r_t = in_h2_r[in_strides[5]];
        DOUBLE v6i_t = in_h2_i[in_strides[5]];
        v6r_t = -v6r_t;
        v6r = v6r_t;
        v6i = v6i_t;

        // Input point 7: x(6)
        DOUBLE v7r_t = in_h2_r[in_strides[6]];
        DOUBLE v7i_t = in_h2_i[in_strides[6]];
        v7r_t = -v7r_t;
        v7r = v7r_t;
        v7i = v7i_t;

        // Input point 8: x(7)
        DOUBLE v8r_t = in_h2_r[in_strides[7]];
        DOUBLE v8i_t = in_h2_i[in_strides[7]];
        v8r_t = -v8r_t;
        v8r = v8r_t;
        v8i = v8i_t;

        // Input point 9: x(8)
        DOUBLE v9r_t = in_h2_r[in_strides[8]];
        DOUBLE v9i_t = in_h2_i[in_strides[8]];
        v9r_t = -v9r_t;
        v9r = v9r_t;
        v9i = v9i_t;

        // Input points - Real : 0, 3, 6
        v1rr = v4r + v7r;
        tv1rr3 = CRTM_9_8 * (v7r - v4r);
        cv1rr = v1r + v1rr;
        tv2rr3 = v1r - (CRTM_9_7 * v1rr);

        // Input points - Real : 1, 4, 7
        v1rr = v5r + v8r;
        tv1rr1 = CRTM_9_8 * (v8r - v5r);
        cv4rr = v2r + v1rr;
        tv2rr1 = v2r - (CRTM_9_7 * v1rr);

        // Input points - Real : 2, 5, 8
        v1rr = v6r + v9r;
        tv1rr2 = CRTM_9_8 * (v9r - v6r);
        cv7rr = v3r + v1rr;
        tv2rr2 = v3r - (CRTM_9_7 * v1rr);

        // Input points - Imag : 0, 3, 6
        v1ii = v4i + v7i;
        tv1ii3 = CRTM_9_8 * (v4i - v7i);
        cv1ii = v1i + v1ii;
        tv2ii3 = v1i - (CRTM_9_7 * v1ii);

        // Input points - Imag : 1, 4, 7
        v1ii = v5i + v8i;
        tv1ii1 = CRTM_9_8 * (v5i - v8i);
        cv4ii = v2i + v1ii;
        tv2ii1 = v2i - (CRTM_9_7 * v1ii);

        // Input points - Imag : 2, 5, 8
        v1ii = v6i + v9i;
        tv1ii2 = CRTM_9_8 * (v6i - v9i);
        cv7ii = v3i + v1ii;
        tv2ii2 = v3i - (CRTM_9_7 * v1ii);

        // Output point : 0
        v1rr = cv4rr + cv7rr;
        tv1rr = CRTM_9_8 * (cv7rr - cv4rr);
        v1ii = cv4ii + cv7ii;
        tv1ii = CRTM_9_8 * (cv4ii - cv7ii);

        *out_h1_r = cv1rr + v1rr;
        tv2rr = cv1rr - (CRTM_9_7 * v1rr);
        *out_h1_i = cv1ii + v1ii;
        tv2ii = cv1ii - (CRTM_9_7 * v1ii);

        // Output points : 3, 6
        {
            UINTP _twa = DATA_STRIDE * (3 * cols + cnt * load_multi_cols);
            DOUBLE _twr = tw[_twa];
            DOUBLE _twi = tw[1 + _twa];
            DOUBLE _or_3 = tv2rr + tv1ii;
            DOUBLE _oi_3 = tv2ii + tv1rr;
            out_h1_r[out_strides[3]] = _or_3 * _twr + _oi_3 * _twi;
            out_h1_i[out_strides[3]] = _oi_3 * _twr - _or_3 * _twi;
        }
        DOUBLE _or_6 = tv2rr - tv1ii;
        {
            UINTP _twa = DATA_STRIDE * (6 * cols + cnt * load_multi_cols);
            DOUBLE _twr = tw[_twa];
            DOUBLE _twi = tw[1 + _twa];
            DOUBLE _oi = tv2ii - tv1rr;
            out_h2_r[out_strides[6]] = _or_6 * _twr + _oi * _twi;
            out_h2_i[out_strides[6]] = _oi * _twr - _or_6 * _twi;
        }

        cv5rr = tv2rr1 + tv1ii1;
        cv6rr = tv2rr1 - tv1ii1;
        cv5ii = tv2ii1 + tv1rr1;
        cv6ii = tv2ii1 - tv1rr1;
        tv3ir = (CRTM_9_6 * cv5rr) + (CRTM_9_5 * cv5ii);
        tv3ir1 = (CRTM_9_6 * cv5ii) - (CRTM_9_5 * cv5rr);

        cv8rr = tv2rr2 + tv1ii2;
        cv9rr = tv2rr2 - tv1ii2;
        cv8ii = tv2ii2 + tv1rr2;
        cv9ii = tv2ii2 - tv1rr2;
        tv4ir = (CRTM_9_4 * cv8rr) + (CRTM_9_3 * cv8ii);
        tv4ir1 = (CRTM_9_4 * cv8ii) - (CRTM_9_3 * cv8rr);

        tv3rr = CRTM_9_8 * (tv4ir - tv3ir);
        tv1rr = tv3ir + tv4ir;
        tv3ii = CRTM_9_8 * (tv3ir1 - tv4ir1);
        tv1ii = tv3ir1 + tv4ir1;
        cv3rr = tv2rr3 - tv1ii3;
        cv3ii = tv2ii3 - tv1rr3;
        cv2rr = tv2rr3 + tv1ii3;
        cv2ii = tv2ii3 + tv1rr3;

        // Output point : 1
        {
            UINTP _twa = DATA_STRIDE * (1 * cols + cnt * load_multi_cols);
            DOUBLE _twr = tw[_twa];
            DOUBLE _twi = tw[1 + _twa];
            DOUBLE _or_1 = cv2rr + tv1rr;
            DOUBLE _oi_1 = cv2ii + tv1ii;
            out_h1_r[out_strides[1]] = _or_1 * _twr + _oi_1 * _twi;
            out_h1_i[out_strides[1]] = _oi_1 * _twr - _or_1 * _twi;
        }

        tv2rr = cv2rr - (CRTM_9_7 * tv1rr);
        tv2ii = cv2ii - (CRTM_9_7 * tv1ii);

        // Output point : 4
        {
            UINTP _twa = DATA_STRIDE * (4 * cols + cnt * load_multi_cols);
            DOUBLE _twr = tw[_twa];
            DOUBLE _twi = tw[1 + _twa];
            DOUBLE _or_4 = tv2rr + tv3ii;
            DOUBLE _oi_4 = tv2ii + tv3rr;
            out_h1_r[out_strides[4]] = _or_4 * _twr + _oi_4 * _twi;
            out_h1_i[out_strides[4]] = _oi_4 * _twr - _or_4 * _twi;
        }

        // Output point : 7
        DOUBLE _or_7 = tv2rr - tv3ii;
        {
            UINTP _twa = DATA_STRIDE * (7 * cols + cnt * load_multi_cols);
            DOUBLE _twr = tw[_twa];
            DOUBLE _twi = tw[1 + _twa];
            DOUBLE _oi = tv2ii - tv3rr;
            out_h2_r[out_strides[7]] = _or_7 * _twr + _oi * _twi;
            out_h2_i[out_strides[7]] = _oi * _twr - _or_7 * _twi;
        }

        tv3ir = (CRTM_9_4 * cv6rr) + (CRTM_9_3 * cv6ii);
        tv3ir1 = (CRTM_9_4 * cv6ii) - (CRTM_9_3 * cv6rr);
        tv4ir = (CRTM_9_2 * cv9ii) - (CRTM_9_1 * cv9rr);
        tv4ir1 = (CRTM_9_1 * cv9ii) + (CRTM_9_2 * cv9rr);
        tv3rr = CRTM_9_8 * (tv4ir - tv3ir);
        tv1rr = tv3ir + tv4ir;
        tv3ii = CRTM_9_8 * (tv3ir1 + tv4ir1);
        tv1ii = tv3ir1 - tv4ir1;

        // Output point : 2
        {
            UINTP _twa = DATA_STRIDE * (2 * cols + cnt * load_multi_cols);
            DOUBLE _twr = tw[_twa];
            DOUBLE _twi = tw[1 + _twa];
            DOUBLE _or_2 = cv3rr + tv1rr;
            DOUBLE _oi_2 = cv3ii + tv1ii;
            out_h1_r[out_strides[2]] = _or_2 * _twr + _oi_2 * _twi;
            out_h1_i[out_strides[2]] = _oi_2 * _twr - _or_2 * _twi;
        }

        tv2rr = cv3rr - (CRTM_9_7 * tv1rr);
        tv2ii = cv3ii - (CRTM_9_7 * tv1ii);

        // Output point : 5
        DOUBLE _or_5 = tv2rr + tv3ii;
        {
            UINTP _twa = DATA_STRIDE * (5 * cols + cnt * load_multi_cols);
            DOUBLE _twr = tw[_twa];
            DOUBLE _twi = tw[1 + _twa];
            DOUBLE _oi = tv2ii + tv3rr;
            out_h2_r[out_strides[5]] = _or_5 * _twr + _oi * _twi;
            out_h2_i[out_strides[5]] = _oi * _twr - _or_5 * _twi;
        }

        // Output point : 8
        DOUBLE _or_8 = tv2rr - tv3ii;
        {
            UINTP _twa = DATA_STRIDE * (8 * cols + cnt * load_multi_cols);
            DOUBLE _twr = tw[_twa];
            DOUBLE _twi = tw[1 + _twa];
            DOUBLE _oi = tv2ii - tv3rr;
            out_h2_r[out_strides[8]] = _or_8 * _twr + _oi * _twi;
            out_h2_i[out_strides[8]] = _oi * _twr - _or_8 * _twi;
        }

        in_h1_r = in_h1_r + v_in_stride;
        in_h2_r = in_h2_r + v_in_h2_stride;
        in_h1_i = in_h1_i + v_in_stride;
        in_h2_i = in_h2_i + v_in_h2_stride;
        out_h1_r = out_h1_r + v_out_stride;
        out_h2_r = out_h2_r + v_out_h2_stride;
        out_h1_i = out_h1_i + v_out_stride;
        out_h2_i = out_h2_i + v_out_h2_stride;
    }
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Exit");
}

static VOID twid_c2r_fft9c_fp32(VOID *in_real, VOID *in_imag, VOID *out_real,
                                VOID *out_imag, INTP n,
                                aoclfftz_strides_t *strides, VOID *twd,
                                UINT8 flag)
{
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Enter");
    const FLOAT CRTM_9_1 = +0.939692620785908384054109277324731469936208134f;
    const FLOAT CRTM_9_2 = +0.342020143325668733044099614682259580763083368f;
    const FLOAT CRTM_9_3 = +0.984807753012208059366743024589523013670643252f;
    const FLOAT CRTM_9_4 = +0.173648177666930348851716626769314796000375677f;
    const FLOAT CRTM_9_5 = +0.642787609686539326322643409907263432907559884f;
    const FLOAT CRTM_9_6 = +0.766044443118978035202392650555416673935832457f;
    const FLOAT CRTM_9_7 = +0.500000000000000000000000000000000000000000000f;
    const FLOAT CRTM_9_8 = +0.866025403784438646763723170752936183471402627f;

    FLOAT *in_h1_r, *in_h2_r, *in_h1_i, *in_h2_i, *out_h1_r, *out_h2_r,
        *out_h1_i, *out_h2_i;
#ifdef VOLATILE_STRIDE_ARRAY
    volatile INTP *in_strides = strides->in_strides;
    volatile INTP *out_strides = strides->out_strides;
#else
    INTP *in_strides = strides->in_strides;
    INTP *out_strides = strides->out_strides;
#endif
    INTP v_in_stride = strides->v_in_stride;
    INTP v_in_h2_stride = strides->v_in_h2_stride;
    INTP v_out_stride = strides->v_out_stride;
    INTP v_out_h2_stride = strides->v_out_h2_stride;

    aoclfftz_twiddle_t *tws = (aoclfftz_twiddle_t *)twd;
    FLOAT *tw = (FLOAT *)(tws->TW);
    UINTP cols = tws->cols;
    UINTP load_multi_cols = tws->load_multi_cols;

    in_h1_r = (FLOAT *)in_imag;
    in_h2_r = in_h1_r;
    in_h1_i = (FLOAT *)in_real;
    in_h2_i = in_h1_i;
    out_h1_r = (FLOAT *)out_imag;
    out_h2_r = out_h1_r;
    out_h1_i = (FLOAT *)out_real;
    out_h2_i = out_h1_i;

    for (INTP cnt = 0; cnt < n; cnt++)
    {
        FLOAT v1r, v1i, v2r, v2i, v3r, v3i, v4r, v4i, v5r, v5i, v6r, v6i, v7r,
            v7i, v8r, v8i, v9r, v9i;
        FLOAT v1rr, v1ii, tv1rr, tv1ii, tv2rr, tv2ii, cv1rr, cv1ii, cv2rr,
            cv2ii, cv3rr, cv3ii, cv4rr, cv4ii, cv5rr, cv5ii, cv6rr, cv6ii,
            cv7rr, cv7ii, cv8rr, cv8ii, cv9rr, cv9ii;
        FLOAT tv1rr1, tv1rr2, tv1rr3, tv1ii1, tv1ii2, tv1ii3;
        FLOAT tv2rr1, tv2rr2, tv2rr3, tv2ii1, tv2ii2, tv2ii3;
        FLOAT tv3rr, tv3ii, tv3ir, tv3ir1, tv4ir, tv4ir1;

        v1r = *in_h1_r;
        v1i = *in_h1_i;

        // Input point 2: x(1)
        FLOAT v2r_t = in_h1_r[in_strides[1]];
        FLOAT v2i_t = in_h1_i[in_strides[1]];
        v2r = v2r_t;
        v2i = v2i_t;

        // Input point 3: x(2)
        FLOAT v3r_t = in_h1_r[in_strides[2]];
        FLOAT v3i_t = in_h1_i[in_strides[2]];
        v3r = v3r_t;
        v3i = v3i_t;

        // Input point 4: x(3)
        FLOAT v4r_t = in_h1_r[in_strides[3]];
        FLOAT v4i_t = in_h1_i[in_strides[3]];
        v4r = v4r_t;
        v4i = v4i_t;

        // Input point 5: x(4)
        FLOAT v5r_t = in_h1_r[in_strides[4]];
        FLOAT v5i_t = in_h1_i[in_strides[4]];
        v5r = v5r_t;
        v5i = v5i_t;

        // Input point 6: x(5)
        FLOAT v6r_t = in_h2_r[in_strides[5]];
        FLOAT v6i_t = in_h2_i[in_strides[5]];
        v6r_t = -v6r_t;
        v6r = v6r_t;
        v6i = v6i_t;

        // Input point 7: x(6)
        FLOAT v7r_t = in_h2_r[in_strides[6]];
        FLOAT v7i_t = in_h2_i[in_strides[6]];
        v7r_t = -v7r_t;
        v7r = v7r_t;
        v7i = v7i_t;

        // Input point 8: x(7)
        FLOAT v8r_t = in_h2_r[in_strides[7]];
        FLOAT v8i_t = in_h2_i[in_strides[7]];
        v8r_t = -v8r_t;
        v8r = v8r_t;
        v8i = v8i_t;

        // Input point 9: x(8)
        FLOAT v9r_t = in_h2_r[in_strides[8]];
        FLOAT v9i_t = in_h2_i[in_strides[8]];
        v9r_t = -v9r_t;
        v9r = v9r_t;
        v9i = v9i_t;

        // Input points - Real : 0, 3, 6
        v1rr = v4r + v7r;
        tv1rr3 = CRTM_9_8 * (v7r - v4r);
        cv1rr = v1r + v1rr;
        tv2rr3 = v1r - (CRTM_9_7 * v1rr);

        // Input points - Real : 1, 4, 7
        v1rr = v5r + v8r;
        tv1rr1 = CRTM_9_8 * (v8r - v5r);
        cv4rr = v2r + v1rr;
        tv2rr1 = v2r - (CRTM_9_7 * v1rr);

        // Input points - Real : 2, 5, 8
        v1rr = v6r + v9r;
        tv1rr2 = CRTM_9_8 * (v9r - v6r);
        cv7rr = v3r + v1rr;
        tv2rr2 = v3r - (CRTM_9_7 * v1rr);

        // Input points - Imag : 0, 3, 6
        v1ii = v4i + v7i;
        tv1ii3 = CRTM_9_8 * (v4i - v7i);
        cv1ii = v1i + v1ii;
        tv2ii3 = v1i - (CRTM_9_7 * v1ii);

        // Input points - Imag : 1, 4, 7
        v1ii = v5i + v8i;
        tv1ii1 = CRTM_9_8 * (v5i - v8i);
        cv4ii = v2i + v1ii;
        tv2ii1 = v2i - (CRTM_9_7 * v1ii);

        // Input points - Imag : 2, 5, 8
        v1ii = v6i + v9i;
        tv1ii2 = CRTM_9_8 * (v6i - v9i);
        cv7ii = v3i + v1ii;
        tv2ii2 = v3i - (CRTM_9_7 * v1ii);

        // Output point : 0
        v1rr = cv4rr + cv7rr;
        tv1rr = CRTM_9_8 * (cv7rr - cv4rr);
        v1ii = cv4ii + cv7ii;
        tv1ii = CRTM_9_8 * (cv4ii - cv7ii);

        *out_h1_r = cv1rr + v1rr;
        tv2rr = cv1rr - (CRTM_9_7 * v1rr);
        *out_h1_i = cv1ii + v1ii;
        tv2ii = cv1ii - (CRTM_9_7 * v1ii);

        // Output points : 3, 6
        {
            UINTP _twa = DATA_STRIDE * (3 * cols + cnt * load_multi_cols);
            FLOAT _twr = tw[_twa];
            FLOAT _twi = tw[1 + _twa];
            FLOAT _or_3 = tv2rr + tv1ii;
            FLOAT _oi_3 = tv2ii + tv1rr;
            out_h1_r[out_strides[3]] = _or_3 * _twr + _oi_3 * _twi;
            out_h1_i[out_strides[3]] = _oi_3 * _twr - _or_3 * _twi;
        }
        FLOAT _or_6 = tv2rr - tv1ii;
        {
            UINTP _twa = DATA_STRIDE * (6 * cols + cnt * load_multi_cols);
            FLOAT _twr = tw[_twa];
            FLOAT _twi = tw[1 + _twa];
            FLOAT _oi = tv2ii - tv1rr;
            out_h2_r[out_strides[6]] = _or_6 * _twr + _oi * _twi;
            out_h2_i[out_strides[6]] = _oi * _twr - _or_6 * _twi;
        }

        cv5rr = tv2rr1 + tv1ii1;
        cv6rr = tv2rr1 - tv1ii1;
        cv5ii = tv2ii1 + tv1rr1;
        cv6ii = tv2ii1 - tv1rr1;
        tv3ir = (CRTM_9_6 * cv5rr) + (CRTM_9_5 * cv5ii);
        tv3ir1 = (CRTM_9_6 * cv5ii) - (CRTM_9_5 * cv5rr);

        cv8rr = tv2rr2 + tv1ii2;
        cv9rr = tv2rr2 - tv1ii2;
        cv8ii = tv2ii2 + tv1rr2;
        cv9ii = tv2ii2 - tv1rr2;
        tv4ir = (CRTM_9_4 * cv8rr) + (CRTM_9_3 * cv8ii);
        tv4ir1 = (CRTM_9_4 * cv8ii) - (CRTM_9_3 * cv8rr);

        tv3rr = CRTM_9_8 * (tv4ir - tv3ir);
        tv1rr = tv3ir + tv4ir;
        tv3ii = CRTM_9_8 * (tv3ir1 - tv4ir1);
        tv1ii = tv3ir1 + tv4ir1;
        cv3rr = tv2rr3 - tv1ii3;
        cv3ii = tv2ii3 - tv1rr3;
        cv2rr = tv2rr3 + tv1ii3;
        cv2ii = tv2ii3 + tv1rr3;

        // Output point : 1
        {
            UINTP _twa = DATA_STRIDE * (1 * cols + cnt * load_multi_cols);
            FLOAT _twr = tw[_twa];
            FLOAT _twi = tw[1 + _twa];
            FLOAT _or_1 = cv2rr + tv1rr;
            FLOAT _oi_1 = cv2ii + tv1ii;
            out_h1_r[out_strides[1]] = _or_1 * _twr + _oi_1 * _twi;
            out_h1_i[out_strides[1]] = _oi_1 * _twr - _or_1 * _twi;
        }

        tv2rr = cv2rr - (CRTM_9_7 * tv1rr);
        tv2ii = cv2ii - (CRTM_9_7 * tv1ii);

        // Output point : 4
        {
            UINTP _twa = DATA_STRIDE * (4 * cols + cnt * load_multi_cols);
            FLOAT _twr = tw[_twa];
            FLOAT _twi = tw[1 + _twa];
            FLOAT _or_4 = tv2rr + tv3ii;
            FLOAT _oi_4 = tv2ii + tv3rr;
            out_h1_r[out_strides[4]] = _or_4 * _twr + _oi_4 * _twi;
            out_h1_i[out_strides[4]] = _oi_4 * _twr - _or_4 * _twi;
        }

        // Output point : 7
        FLOAT _or_7 = tv2rr - tv3ii;
        {
            UINTP _twa = DATA_STRIDE * (7 * cols + cnt * load_multi_cols);
            FLOAT _twr = tw[_twa];
            FLOAT _twi = tw[1 + _twa];
            FLOAT _oi = tv2ii - tv3rr;
            out_h2_r[out_strides[7]] = _or_7 * _twr + _oi * _twi;
            out_h2_i[out_strides[7]] = _oi * _twr - _or_7 * _twi;
        }

        tv3ir = (CRTM_9_4 * cv6rr) + (CRTM_9_3 * cv6ii);
        tv3ir1 = (CRTM_9_4 * cv6ii) - (CRTM_9_3 * cv6rr);
        tv4ir = (CRTM_9_2 * cv9ii) - (CRTM_9_1 * cv9rr);
        tv4ir1 = (CRTM_9_1 * cv9ii) + (CRTM_9_2 * cv9rr);
        tv3rr = CRTM_9_8 * (tv4ir - tv3ir);
        tv1rr = tv3ir + tv4ir;
        tv3ii = CRTM_9_8 * (tv3ir1 + tv4ir1);
        tv1ii = tv3ir1 - tv4ir1;

        // Output point : 2
        {
            UINTP _twa = DATA_STRIDE * (2 * cols + cnt * load_multi_cols);
            FLOAT _twr = tw[_twa];
            FLOAT _twi = tw[1 + _twa];
            FLOAT _or_2 = cv3rr + tv1rr;
            FLOAT _oi_2 = cv3ii + tv1ii;
            out_h1_r[out_strides[2]] = _or_2 * _twr + _oi_2 * _twi;
            out_h1_i[out_strides[2]] = _oi_2 * _twr - _or_2 * _twi;
        }

        tv2rr = cv3rr - (CRTM_9_7 * tv1rr);
        tv2ii = cv3ii - (CRTM_9_7 * tv1ii);

        // Output point : 5
        FLOAT _or_5 = tv2rr + tv3ii;
        {
            UINTP _twa = DATA_STRIDE * (5 * cols + cnt * load_multi_cols);
            FLOAT _twr = tw[_twa];
            FLOAT _twi = tw[1 + _twa];
            FLOAT _oi = tv2ii + tv3rr;
            out_h2_r[out_strides[5]] = _or_5 * _twr + _oi * _twi;
            out_h2_i[out_strides[5]] = _oi * _twr - _or_5 * _twi;
        }

        // Output point : 8
        FLOAT _or_8 = tv2rr - tv3ii;
        {
            UINTP _twa = DATA_STRIDE * (8 * cols + cnt * load_multi_cols);
            FLOAT _twr = tw[_twa];
            FLOAT _twi = tw[1 + _twa];
            FLOAT _oi = tv2ii - tv3rr;
            out_h2_r[out_strides[8]] = _or_8 * _twr + _oi * _twi;
            out_h2_i[out_strides[8]] = _oi * _twr - _or_8 * _twi;
        }

        in_h1_r = in_h1_r + v_in_stride;
        in_h2_r = in_h2_r + v_in_h2_stride;
        in_h1_i = in_h1_i + v_in_stride;
        in_h2_i = in_h2_i + v_in_h2_stride;
        out_h1_r = out_h1_r + v_out_stride;
        out_h2_r = out_h2_r + v_out_h2_stride;
        out_h1_i = out_h1_i + v_out_stride;
        out_h2_i = out_h2_i + v_out_h2_stride;
    }
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Exit");
}

kfft_ register_kernel_twid_c2r_fft9c(UINT8 precision,
                                     UINT8 direction /* unused */)
{
    if (precision == DT_FLOAT)
    {
        return twid_c2r_fft9c_fp32;
    }
    else if (precision == DT_DOUBLE)
    {
        return twid_c2r_fft9c_fp64;
    }
    else
    {
        return NULL;
    }
}
