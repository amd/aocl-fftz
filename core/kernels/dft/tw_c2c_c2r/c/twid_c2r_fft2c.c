...
static FFTZ_VOID twid_c2r_fft2c_fp32(FFTZ_VOID *in_real, FFTZ_VOID *in_imag,
                                     ...
                                     FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    ...
    for (cnt = 0; cnt < n; cnt++)
    {
        ...
        *out_h1_i = v1i + v2i;
        ...
        // Output point 2: X(1)
        FFTZ_FLOAT _or_1 = v1r - v2r;
        {
            FFTZ_UINTP _twa = DATA_STRIDE * (1 * cols + cnt * load_multi_cols);
            FFTZ_FLOAT _twr = tw[_twa];
            FFTZ_FLOAT _twi = tw[1 + _twa];
            FFTZ_FLOAT _oi = v1i - v2i;
            out_h2_r[out_strides[1]] = _or_1 * _twr - _oi * _twi;
            out_h2_i[out_strides[1]] = _or_1 * _twi + _oi * _twr;
        }
...
static FFTZ_VOID twid_c2r_fft2c_fp64(FFTZ_VOID *in_real, FFTZ_VOID *in_imag,
                                     FFTZ_VOID *out_real, FFTZ_VOID *out_imag,
                                     FFTZ_INTP n, aoclfftz_strides_t *strides,
                                     FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
                                     ...
                                     FFTZ_UINT8 direction /* unused */)
{
    if (precision == DT_FLOAT)
    {
        return twid_c2r_fft2c_fp32;
    }
    else if (precision == DT_DOUBLE)
    {
        return twid_c2r_fft2c_fp64;
    }
    else
    {
        return NULL;
