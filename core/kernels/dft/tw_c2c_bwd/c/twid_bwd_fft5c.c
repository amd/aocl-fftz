...
/** @file twid_bwd_fft5c.c
 *
 *  @brief Backward-only twiddle Radix-5 FFT kernel with scalar operations in C
 *
...
static VOID twid_bwd_fft5c_fp64(VOID *in_real, VOID *in_imag, VOID *out_real,
                                ...
                                UINT8 flag)
{
    ...
    for (cnt = 0; cnt < n; cnt++)
    {
        ...
        *out_h1_i = v1i + cv1ii;
        ...
        out_h1_r[out_strides[1]] = cv3rr - tvri;
        out_h1_i[out_strides[1]] = cv3ii + tvir;

        // Output point 5: X(4)
        out_h2_r[out_strides[4]] = cv3rr + tvri;
        out_h2_i[out_strides[4]] = cv3ii - tvir;
        ...
        out_h1_r[out_strides[2]] = cv3rr - tvri;
        out_h1_i[out_strides[2]] = cv3ii + tvir;

        // Output point 4: X(3)
        out_h2_r[out_strides[3]] = cv3rr + tvri;
        out_h2_i[out_strides[3]] = cv3ii - tvir;
...
static VOID twid_bwd_fft5c_fp32(VOID *in_real, VOID *in_imag, VOID *out_real,
                                VOID *out_imag, INTP n,
                                aoclfftz_strides_t *strides, VOID *twd,
                                UINT8 flag)
{
...
kfft_ register_kernel_twid_bwd_fft5c(UINT8 precision,
                                     UINT8 direction /* unused */)
{
    if (precision == DT_FLOAT)
    {
        return twid_bwd_fft5c_fp32;
    }
    else if (precision == DT_DOUBLE)
    {
        return twid_bwd_fft5c_fp64;
    }
    else
    {
        return NULL;
