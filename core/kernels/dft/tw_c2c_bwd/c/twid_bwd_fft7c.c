...
/** @file twid_bwd_fft7c.c
 *
 *  @brief Backward-only twiddle Radix-7 FFT kernel with scalar operations in C
 *
...
static VOID twid_bwd_fft7c_fp64(VOID *in_real, VOID *in_imag, VOID *out_real,
                                ...
                                UINT8 flag)
{
    ...
    for (INTP cnt = 0; cnt < n; cnt++)
    {
        ...
        *out_h1_i = v1i + av1ir + av2ir + av3ir;
        ...
        out_h1_r[out_strides[1]] = cvrr - cvri;
        out_h2_r[out_strides[6]] = cvrr + cvri;
        ...
        out_h1_i[out_strides[1]] = cvir + cvii;
        out_h2_i[out_strides[6]] = cvii - cvir;
        ...
        out_h1_r[out_strides[2]] = cvrr - cvri;
        out_h2_r[out_strides[5]] = cvrr + cvri;
        ...
        out_h1_i[out_strides[2]] = cvii + cvir;
        out_h2_i[out_strides[5]] = cvii - cvir;
        ...
        cvri = tv1ri - tv2ri + tv3ri;
        out_h1_r[out_strides[3]] = cvrr - cvri;
        out_h2_r[out_strides[4]] = cvrr + cvri;
        ...
        cvir = tv1ir - tv2ir + tv3ir;
        out_h1_i[out_strides[3]] = cvii + cvir;
        out_h2_i[out_strides[4]] = cvii - cvir;
...
static VOID twid_bwd_fft7c_fp32(VOID *in_real, VOID *in_imag, VOID *out_real,
                                VOID *out_imag, INTP n,
                                aoclfftz_strides_t *strides, VOID *twd,
                                UINT8 flag)
{
...
kfft_ register_kernel_twid_bwd_fft7c(UINT8 precision,
                                     UINT8 direction /* unused */)
{
    if (precision == DT_FLOAT)
    {
        return twid_bwd_fft7c_fp32;
    }
    else if (precision == DT_DOUBLE)
    {
        return twid_bwd_fft7c_fp64;
    }
    else
    {
        return NULL;
