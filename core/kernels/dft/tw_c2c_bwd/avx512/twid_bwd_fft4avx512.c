// Copyright Advanced Micro Devices, Inc.
// SPDX-License-Identifier: BSD-3-Clause

/** @file twid_bwd_fft4avx512.c
 *
 *  @brief Backward-only twiddle Radix-4 FFT kernel with avx512 operations using
 *  x86 SIMD intrinsics
 *
 *  This file contains the DIT twiddle radix-4 FFT implementations using AVX-512
 *  operations for single-precision and double-precision inputs.
 *
 *  @author Ashwin K. Godbole
 *  @author Srirammaswamy Srinivasan
 */

#include "core/kernels/kernel.h"
#include "core/kernels/simd_includes/simd_common_avx512.h"

static const ops_cycles_t ops_cnt[NUM_PRECISIONS] = {{3, 7, 8, 70, 31, 25},
                                                     {3, 7, 8, 38, 13, 25}};

#define TWID_KNAME_FP32 twid_bwd_fft4avx512fp32
#define TWID_KNAME_FP64 twid_bwd_fft4avx512fp64
#define GET_OPS_COUNT get_ops_cnt_twid_bwd_fft4avx512
#define REGISTER_KERNEL register_kernel_twid_bwd_fft4avx512
#define KERNEL_USE_AVX512

#define LOAD_IN_S ITW_GATHER_S
#define LOAD_IN_256_S ITW_GATHER_256_S
#define LOAD_IN_128_S ITW_GATHER_128_S
#define LOAD_IN_64_S ITW_GATHER_LOW_128_S
#define LOAD_IN_D ITW_GATHER_D
#define LOAD_IN_256_D ITW_GATHER_256_D
#define LOAD_IN_128_D ITW_GATHER_128_D

#define STORE_OUT_S SCATTER_NOTW_S
#define STORE_OUT_256_S SCATTER_NOTW_256_S
#define STORE_OUT_128_S SCATTER_NOTW_128_S
#define STORE_OUT_64_S SCATTER_NOTW_LOW_128_S
#define STORE_OUT_D SCATTER_NOTW_D
#define STORE_OUT_256_D SCATTER_NOTW_256_D
#define STORE_OUT_128_D SCATTER_NOTW_128_D

#define KERNEL_VARIANT_C2C
#define KERNEL_DIRECTION_BWD

#define TWID_PRELOADED_LOAD_D ITW_PRELOADED_GATHER_D_V
#define TWID_PRELOADED_LOAD_256_D ITW_PRELOADED_GATHER_256_D_V
#define TWID_PRELOADED_LOAD_128_D ITW_PRELOADED_GATHER_128_D_V
#define TWID_PRELOADED_STORE_D PRELOADED_SCATTER_NOTW_D
#define TWID_PRELOADED_STORE_256_D PRELOADED_SCATTER_NOTW_256_D
#define TWID_PRELOADED_STORE_128_D PRELOADED_SCATTER_NOTW_128_D

#include "core/kernels/dft/twid_c2c_common/twid_fft4.h"

#undef TWID_PRELOADED_STORE_128_D
#undef TWID_PRELOADED_STORE_256_D
#undef TWID_PRELOADED_STORE_D
#undef TWID_PRELOADED_LOAD_128_D
#undef TWID_PRELOADED_LOAD_256_D
#undef TWID_PRELOADED_LOAD_D

#undef KERNEL_DIRECTION_BWD
#undef KERNEL_VARIANT_C2C

#undef STORE_OUT_128_D
#undef STORE_OUT_256_D
#undef STORE_OUT_D
#undef STORE_OUT_64_S
#undef STORE_OUT_128_S
#undef STORE_OUT_256_S
#undef STORE_OUT_S

#undef LOAD_IN_128_D
#undef LOAD_IN_256_D
#undef LOAD_IN_D
#undef LOAD_IN_64_S
#undef LOAD_IN_128_S
#undef LOAD_IN_256_S
#undef LOAD_IN_S

#undef KERNEL_USE_AVX512
#undef REGISTER_KERNEL
#undef GET_OPS_COUNT
#undef TWID_KNAME_FP64
#undef TWID_KNAME_FP32
