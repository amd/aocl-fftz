...
/** @file fft6avx256.c
 *
 *  @brief Radix-6 FFT kernel with AVX-256 operations using x86 SIMD intrinsics
 *
 *  This file contains the DIT radix-6 FFT implementations using AVX-256
...
static const ops_cycles_t ops_cnt[NUM_PRECISIONS] = {{0, 4, 18, 48, 8, 14},
                                                     {0, 4, 18, 24, 2, 14}};
...
static VOID fft6avx256fp32(VOID *in_real, VOID *in_imag, VOID *out_real,
                           ...
                           VOID *twd, UINT8 flag)
{
    ...
    for (count = 0; count < N; count++)
    {
        ...
        v_cv1 = _mm256_sub_ps(v_av1, _mm256_mul_ps(v_C1, v_av4));
        ...
        v_cv2 = _mm256_sub_ps(v_av2, _mm256_mul_ps(v_C1, v_av1));
        ...
        v_cv3 = _mm256_mul_ps(v_C2, v_av3);
...
static VOID fft6avx256fp64(VOID *in_real, VOID *in_imag, VOID *out_real,
                           VOID *out_imag, INTP n, aoclfftz_strides_t *strides,
                           VOID *twd, UINT8 flag)
{
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Enter");
    ...
    for (count = 0; count < N; count++)
    {
        ...
        v_cv1 = _mm256_sub_pd(v_av1, _mm256_mul_pd(v_C1, v_av4));
        ...
        v_cv2 = _mm256_sub_pd(v_av2, _mm256_mul_pd(v_C1, v_av1));
        ...
        v_cv3 = _mm256_mul_pd(v_C2, v_av3);
...
kfft_ register_kernel_fft6avx256(UINT8 precision, UINT8 direction /* unused */)
{
    if (precision == DT_FLOAT)
    {
        return fft6avx256fp32;
    }
    else if (precision == DT_DOUBLE)
    {
        return fft6avx256fp64;
    }
    else
    {
        return NULL;
