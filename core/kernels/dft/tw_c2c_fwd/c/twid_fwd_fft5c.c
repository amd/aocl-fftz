...
/** @file twid_fwd_fft5c.c
 *
 *  @brief Forward-only twiddle Radix-5 FFT kernel with scalar operations in C
 *
...
static VOID twid_fwd_fft5c_fp64(VOID *in_real, VOID *in_imag, VOID *out_real,
...
static VOID twid_fwd_fft5c_fp32(VOID *in_real, VOID *in_imag, VOID *out_real,
                                VOID *out_imag, INTP n,
                                aoclfftz_strides_t *strides, VOID *twd,
                                UINT8 flag)
{
...
kfft_ register_kernel_twid_fwd_fft5c(UINT8 precision,
                                     UINT8 direction /* unused */)
{
    if (precision == DT_FLOAT)
    {
        return twid_fwd_fft5c_fp32;
    }
    else if (precision == DT_DOUBLE)
    {
        return twid_fwd_fft5c_fp64;
    }
    else
    {
        return NULL;
