...
/** @file rfft6avx256.c
 *
 *  @brief Radix-6 r2hc Real-FFT kernel with AVX-256 operations using x86 SIMD
 *  intrinsics
 *
...
static FFTZ_VOID r2hc_rfft6avx256_fp32_fwd(FFTZ_VOID *in_real,
                                           ...
                                           FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    ...
    for (cnt = 0; cnt < N; cnt++)
    {
        ...
        // Output point 2: X(1)
        v_out1 = _mm256_add_ps(v_s1, _mm256_mul_ps(v_CRTM_6_1, v_s7));

        // Output point 3: X(2)
        v_out2 = _mm256_mul_ps(v_CRTM_6_2, _mm256_sub_ps(v_s4, v_s2));

        // Output point 4: X(3)
        v_out3 = _mm256_sub_ps(v_s0, _mm256_mul_ps(v_CRTM_6_1, v_s6));

        // Output point 5: X(4)
        v_out4 = _mm256_mul_ps(v_CRTM_6_2, _mm256_add_ps(v_s5, v_s3));
    ...
    if (remaining_sets & 1)
    {
        ...
        *out_r = s0 + s6;
        ...
        // Output point 2: X(1)
        out_cp[out_strides[1]] = s1 + (CRTM_6_1 * s7);

        // Output point 3: X(2)
        out_cp[out_strides[2]] = CRTM_6_2 * (s4 - s2);

        // Output point 4: X(3)
        out_cp[out_strides[3]] = s0 - (CRTM_6_1 * s6);

        // Output point 5: X(4)
        out_cp[out_strides[4]] = CRTM_6_2 * (s5 + s3);

        // Output point 6: X(5)
        out_r[out_strides[5]] = s1 - s7;
    }
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Exit");
}

static FFTZ_VOID r2hc_rfft6avx256_fp32_bwd(FFTZ_VOID *in_real,
                                           FFTZ_VOID *in_complex,
                                           FFTZ_VOID *out_real,
                                           FFTZ_VOID *out_complex, FFTZ_INTP n,
                                           aoclfftz_strides_t *strides,
                                           FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    ...
    for (cnt = 0; cnt < N; cnt++)
    {
        ...
        // Input point 6: x(5)
        curr_in = in_r + in_strides[5];
        LDR_256_S(curr_in, v_in_dc_nyq_stride, v_in5, is_contiguous_in_dc_nyq);
        ...
        v_t0 = _mm256_mul_ps(v_CRTM_6_1, v_s4);
        v_t1 = _mm256_mul_ps(v_CRTM_6_1, v_s5);
    ...
    if (remaining_sets & NUM_SETS_REAL_128_S)
    {
        ...
        // Input point 6: x(5)
        curr_in = in_r + in_strides[5];
        LDR_128_S(curr_in, v_in_dc_nyq_stride, v_in5, is_contiguous_in_dc_nyq);
    ...
    if (remaining_sets & 2)
    {
        ...
        // Input point 6: x(5)
        curr_in = in_r + in_strides[5];
        LDHR_128_S(curr_in, v_in_dc_nyq_stride, v_in5);
    ...
    if (remaining_sets & 1)
    {
        ...
        *out_r = s0 + (2 * s2);
        ...
        // Output point 2: X(1)
        out_r[out_strides[1]] = s6 - t0;

        // Output point 3: X(2)
        out_r[out_strides[2]] = s7 - t1;

        // Output point 4: X(3)
        out_r[out_strides[3]] = s1 - (2 * s3);

        // Output point 5: X(4)
        out_r[out_strides[4]] = s7 + t1;

        // Output point 6: X(5)
        out_r[out_strides[5]] = s6 + t0;
    }
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Exit");
}

static FFTZ_VOID r2hc_rfft6avx256_fp64_fwd(FFTZ_VOID *in_real,
                                           FFTZ_VOID *in_complex,
                                           FFTZ_VOID *out_real,
                                           FFTZ_VOID *out_complex, FFTZ_INTP n,
                                           aoclfftz_strides_t *strides,
                                           FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    ...
    for (cnt = 0; cnt < N; cnt++)
    {
        ...
        // Output point 2: X(1)
        v_out1 = _mm256_add_pd(v_s1, _mm256_mul_pd(v_CRTM_6_1, v_s7));

        // Output point 3: X(2)
        v_out2 = _mm256_mul_pd(v_CRTM_6_2, _mm256_sub_pd(v_s4, v_s2));

        // Output point 4: X(3)
        v_out3 = _mm256_sub_pd(v_s0, _mm256_mul_pd(v_CRTM_6_1, v_s6));

        // Output point 5: X(4)
        v_out4 = _mm256_mul_pd(v_CRTM_6_2, _mm256_add_pd(v_s5, v_s3));
    ...
    if (remaining_sets & 1)
    {
        ...
        *out_r = s0 + s6;
        ...
        // Output point 5: X(4)
        out_cp[out_strides[4]] = CRTM_6_2 * (s3 + s5);
...
static FFTZ_VOID r2hc_rfft6avx256_fp64_bwd(FFTZ_VOID *in_real,
                                           FFTZ_VOID *in_complex,
                                           FFTZ_VOID *out_real,
                                           FFTZ_VOID *out_complex, FFTZ_INTP n,
                                           aoclfftz_strides_t *strides,
                                           FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    ...
    for (cnt = 0; cnt < N; cnt++)
    {
        ...
        // Input point 6: x(5)
        curr_in = in_r + in_strides[5];
        LDR_256_D(curr_in, v_in_dc_nyq_stride, v_in5, is_contiguous_in_dc_nyq);
        ...
        v_t0 = _mm256_mul_pd(v_CRTM_6_1, v_s4);
        v_t1 = _mm256_mul_pd(v_CRTM_6_1, v_s5);
    ...
    if (remaining_sets & NUM_SETS_REAL_128_D)
    {
        ...
        // Input point 6: x(5)
        curr_in = in_r + in_strides[5];
        LDR_128_D(curr_in, v_in_dc_nyq_stride, v_in5, is_contiguous_in_dc_nyq);
                                       ...
                                       FFTZ_UINT8 direction)
{
    if (direction == FORWARD_FFT_DIR)
    {
        if (precision == DT_FLOAT)
        {
            return r2hc_rfft6avx256_fp32_fwd;
        }
        else if (precision == DT_DOUBLE)
        {
            return r2hc_rfft6avx256_fp64_fwd;
        }
        else
        {
            return NULL;
    ...
    else
    {
        if (precision == DT_FLOAT)
        {
            return r2hc_rfft6avx256_fp32_bwd;
        }
        else if (precision == DT_DOUBLE)
        {
            return r2hc_rfft6avx256_fp64_bwd;
        }
        else
        {
            return NULL;
