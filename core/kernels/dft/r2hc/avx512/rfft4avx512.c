// Copyright Advanced Micro Devices, Inc.
// SPDX-License-Identifier: BSD-3-Clause

/** @file rfft4avx512.c
 *
 *  @brief Radix-4 r2hc Real-FFT kernel with AVX-512 operations using x86 SIMD
 *  intrinsics
 *
 *  This file contains the DIT radix-4 real-to-halfcomplex implementations using
 *  AVX512 SIMD operations for single-precision and double-precision inputs.
 *
 *  @author Jeevanantham N
 */

#include "core/kernels/kernel.h"
#include "core/kernels/simd_includes/r2hc_simd_avx512.h"

static const ops_cycles_t ops_cnt[NUM_PRECISIONS][NUM_FFT_DIRS] =
                                                    {{{0, 0, 6, 112, 74, 24},
                                                      {0, 2, 6, 112, 82, 24}},
                                                     {{0, 0, 6,  56,  2, 24},
                                                      {0, 2, 6,  56,  2, 24}}};

ops_cycles_t get_ops_cnt_r2hc_rfft4avx512(FFTZ_UINT8 precision,
                                          FFTZ_UINT8 direction)
{
    if (precision == DT_FLOAT)
    {
        if (direction == FORWARD_FFT_DIR)
        {
            return ops_cnt[0][0];
        }
        else
        {
            return ops_cnt[0][1];
        }
    }
    else
    {
        if (direction == FORWARD_FFT_DIR)
        {
            return ops_cnt[1][0];
        }
        else
        {
            return ops_cnt[1][1];
        }
    }
}

static FFTZ_VOID r2hc_rfft4avx512_fp32_fwd(FFTZ_VOID *in_real,
                                           FFTZ_VOID *in_complex,
                                           FFTZ_VOID *out_real,
                                           FFTZ_VOID *out_complex, FFTZ_INTP n,
                                           aoclfftz_strides_t *strides,
                                           FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Enter");

    FFTZ_FLOAT *in_r = (FFTZ_FLOAT *)in_real;
    FFTZ_FLOAT *out_r = (FFTZ_FLOAT *)out_real;
    FFTZ_FLOAT *out_cp = (FFTZ_FLOAT *)out_complex;
#ifdef VOLATILE_STRIDE_ARRAY
    volatile FFTZ_INTP *in_strides = strides->in_strides;
    volatile FFTZ_INTP *out_strides = strides->out_strides;
#else
    FFTZ_INTP *in_strides  = strides->in_strides;
    FFTZ_INTP *out_strides = strides->out_strides;
#endif
    FFTZ_INTP v_in_stride  = strides->v_in_stride;
    FFTZ_INTP v_out_stride = strides->v_out_stride;
    FFTZ_UINT8 is_contiguous_in = (v_in_stride == 1);
    // Used for endpoints: DC and Nyquist.
    FFTZ_INTP v_out_dc_nyq_stride = strides->v_out_sym_stride;
    FFTZ_UINT8 is_contiguous_out_dc_nyq = (v_out_dc_nyq_stride == 1);

    FFTZ_INTP cnt;
    FFTZ_FLOAT *curr_in, *curr_out;
    FFTZ_INTP N = n / NUM_SETS_REAL_512_S;
    FFTZ_INTP remaining_sets = n % NUM_SETS_REAL_512_S;

    for (cnt = 0; cnt < N; cnt++)
    {
        __m512 v_in0, v_in1, v_in2, v_in3;
        __m512 v_s0, v_s1;
        __m512 v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_512_S(curr_in, v_in_stride, v_in0, is_contiguous_in);
        // Input point 2: x(1)
        curr_in = in_r + in_strides[1];
        LDR_512_S(curr_in, v_in_stride, v_in1, is_contiguous_in);
        // Input point 3: x(2)
        curr_in = in_r + in_strides[2];
        LDR_512_S(curr_in, v_in_stride, v_in2, is_contiguous_in);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_512_S(curr_in, v_in_stride, v_in3, is_contiguous_in);

        v_s0 = _mm512_add_ps(v_in0, v_in2);
        v_s1 = _mm512_add_ps(v_in1, v_in3);

        // Output point 1: X(0)
        v_out0 = _mm512_add_ps(v_s0, v_s1);

        // Output point 2: X(1)
        v_out1 = _mm512_sub_ps(v_in0, v_in2);

        // Output point 3: X(2)
        v_out2 = _mm512_sub_ps(v_in3, v_in1);

        // Output point 4: X(3)
        v_out3 = _mm512_sub_ps(v_s0, v_s1);
        STR_512_S(curr_out, v_out_dc_nyq_stride, v_out0, is_contiguous_out_dc_nyq);
        curr_out = out_cp + out_strides[1];
        STRI_2x512_S(curr_out, v_out_stride, v_out1, v_out2);
        curr_out = out_r + out_strides[3];
        STR_512_S(curr_out, v_out_dc_nyq_stride, v_out3, is_contiguous_out_dc_nyq);

        in_r  = in_r + (v_in_stride << 4);
        out_cp = out_cp + (v_out_stride << 4);
        out_r = out_r + (v_out_dc_nyq_stride << 4);
    }
    // tail cases
    if (remaining_sets & NUM_SETS_REAL_256_S)
    {
        __m256 v_in0, v_in1, v_in2, v_in3;
        __m256 v_s0, v_s1;
        __m256 v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_256_S(curr_in, v_in_stride, v_in0, is_contiguous_in);
        // Input point 2: x(1)
        curr_in = in_r + in_strides[1];
        LDR_256_S(curr_in, v_in_stride, v_in1, is_contiguous_in);
        // Input point 3: x(2)
        curr_in = in_r + in_strides[2];
        LDR_256_S(curr_in, v_in_stride, v_in2, is_contiguous_in);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_256_S(curr_in, v_in_stride, v_in3, is_contiguous_in);

        v_s0 = _mm256_add_ps(v_in0, v_in2);
        v_s1 = _mm256_add_ps(v_in1, v_in3);

        // Output point 1: X(0)
        v_out0 = _mm256_add_ps(v_s0, v_s1);

        // Output point 2: X(1)
        v_out1 = _mm256_sub_ps(v_in0, v_in2);

        // Output point 3: X(2)
        v_out2 = _mm256_sub_ps(v_in3, v_in1);

        // Output point 4: X(3)
        v_out3 = _mm256_sub_ps(v_s0, v_s1);
        STR_256_S(curr_out, v_out_dc_nyq_stride, v_out0, is_contiguous_out_dc_nyq);
        curr_out = out_cp + out_strides[1];
        STRI_2x256_S(curr_out, v_out_stride, v_out1, v_out2);
        curr_out = out_r + out_strides[3];
        STR_256_S(curr_out, v_out_dc_nyq_stride, v_out3, is_contiguous_out_dc_nyq);

        in_r  = in_r + (v_in_stride << 3);
        out_cp = out_cp + (v_out_stride << 3);
        out_r = out_r + (v_out_dc_nyq_stride << 3);
    }
    // tail cases
    if (remaining_sets & NUM_SETS_REAL_128_S)
    {
        __m128 v_in0, v_in1, v_in2, v_in3;
        __m128 v_s0, v_s1;
        __m128 v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_128_S(curr_in, v_in_stride, v_in0, is_contiguous_in);
        // Input point 2: x(1)
        curr_in = in_r + in_strides[1];
        LDR_128_S(curr_in, v_in_stride, v_in1, is_contiguous_in);
        // Input point 3: x(2)
        curr_in = in_r + in_strides[2];
        LDR_128_S(curr_in, v_in_stride, v_in2, is_contiguous_in);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_128_S(curr_in, v_in_stride, v_in3, is_contiguous_in);

        v_s0 = _mm_add_ps(v_in0, v_in2);
        v_s1 = _mm_add_ps(v_in1, v_in3);

        // Output point 1: X(0)
        v_out0 = _mm_add_ps(v_s0, v_s1);

        // Output point 2: X(1)
        v_out1 = _mm_sub_ps(v_in0, v_in2);

        // Output point 3: X(2)
        v_out2 = _mm_sub_ps(v_in3, v_in1);

        // Output point 4: X(3)
        v_out3 = _mm_sub_ps(v_s0, v_s1);
        STR_128_S(curr_out, v_out_dc_nyq_stride, v_out0, is_contiguous_out_dc_nyq);
        curr_out = out_cp + out_strides[1];
        STRI_2x128_S(curr_out, v_out_stride, v_out1, v_out2);
        curr_out = out_r + out_strides[3];
        STR_128_S(curr_out, v_out_dc_nyq_stride, v_out3, is_contiguous_out_dc_nyq);

        in_r  = in_r + (v_in_stride << 2);
        out_cp = out_cp + (v_out_stride << 2);
        out_r = out_r + (v_out_dc_nyq_stride << 2);
    }
    // tail cases
    if (remaining_sets & 2)
    {
        __m128 v_in0, v_in1, v_in2, v_in3;
        __m128 v_s0, v_s1;
        __m128 v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDHR_128_S(curr_in, v_in_stride, v_in0);
        // Input point 2: x(1)
        curr_in = in_r + in_strides[1];
        LDHR_128_S(curr_in, v_in_stride, v_in1);
        // Input point 3: x(2)
        curr_in = in_r + in_strides[2];
        LDHR_128_S(curr_in, v_in_stride, v_in2);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDHR_128_S(curr_in, v_in_stride, v_in3);

        v_s0 = _mm_add_ps(v_in0, v_in2);
        v_s1 = _mm_add_ps(v_in1, v_in3);

        // Output point 1: X(0)
        v_out0 = _mm_add_ps(v_s0, v_s1);

        // Output point 2: X(1)
        v_out1 = _mm_sub_ps(v_in0, v_in2);

        // Output point 3: X(2)
        v_out2 = _mm_sub_ps(v_in3, v_in1);

        // Output point 4: X(3)
        v_out3 = _mm_sub_ps(v_s0, v_s1);

        STHR_128_S(curr_out, v_out_dc_nyq_stride, v_out0);
        curr_out = out_cp + out_strides[1];
        STHRI_2x128_S(curr_out, v_out_stride, v_out1, v_out2);
        curr_out = out_r + out_strides[3];
        STHR_128_S(curr_out, v_out_dc_nyq_stride, v_out3);

        in_r  = in_r + (v_in_stride << 1);
        out_cp = out_cp + (v_out_stride << 1);
        out_r = out_r + (v_out_dc_nyq_stride << 1);
    }
    // tail cases
    if (remaining_sets & 1)
    {
        FFTZ_FLOAT in0, in1, in2, in3;
        FFTZ_FLOAT s0, s1;

        curr_in = in_r;
        curr_out = out_r;
        // Input point 1: x(0)
        in0 = *in_r;
        // Input point 2: x(1)
        in1 = in_r[in_strides[1]];
        // Input point 3: x(2)
        in2 = in_r[in_strides[2]];
        // Input point 4: x(3)
        in3 = in_r[in_strides[3]];

        s0 = in0 + in2;
        s1 = in1 + in3;

        // Output point 1: X(0)
        *out_r = s0 + s1;

        // Output point 2: X(1)
        out_cp[out_strides[1]] = in0 - in2;

        // Output point 3: X(2)
        out_cp[out_strides[2]] = in3 - in1;

        // Output point 4: X(3)
        out_r[out_strides[3]] = s0 - s1;
    }
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Exit");
}

static FFTZ_VOID r2hc_rfft4avx512_fp32_bwd(FFTZ_VOID *in_real,
                                           FFTZ_VOID *in_complex,
                                           FFTZ_VOID *out_real,
                                           FFTZ_VOID *out_complex, FFTZ_INTP n,
                                           aoclfftz_strides_t *strides,
                                           FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Enter");
    const FFTZ_FLOAT CRTM_4_1 =
        2.000000000000000000000000000000000000000000000f;

    FFTZ_FLOAT *in_r = (FFTZ_FLOAT *)in_real;
    FFTZ_FLOAT *in_cp = (FFTZ_FLOAT *)in_complex;
    FFTZ_FLOAT *out_r = (FFTZ_FLOAT *)out_real;
#ifdef VOLATILE_STRIDE_ARRAY
    volatile FFTZ_INTP *in_strides = strides->in_strides;
    volatile FFTZ_INTP *out_strides = strides->out_strides;
#else
    FFTZ_INTP *in_strides  = strides->in_strides;
    FFTZ_INTP *out_strides = strides->out_strides;
#endif
    FFTZ_INTP v_in_stride  = strides->v_in_stride;
    FFTZ_INTP v_out_stride = strides->v_out_stride;
    FFTZ_UINT8 is_contiguous_out = (v_out_stride == 1);
    // Used for endpoints: DC and Nyquist.
    FFTZ_INTP v_in_dc_nyq_stride = strides->v_in_sym_stride;
    FFTZ_UINT8 is_contiguous_in_dc_nyq = (v_in_dc_nyq_stride == 1);

    FFTZ_INTP cnt;
    FFTZ_FLOAT *curr_in, *curr_out;
    FFTZ_INTP N = n / NUM_SETS_REAL_512_S;
    FFTZ_INTP remaining_sets = n % NUM_SETS_REAL_512_S;

    __m512 v_CRTM_4_1 = _mm512_set1_ps(CRTM_4_1);

    for (cnt = 0; cnt < N; cnt++)
    {
        __m512 v_in0, v_in1, v_in2, v_in3;
        __m512 v_s0, v_s1;
        __m512 v_t0, v_t1;
        __m512 v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_512_S(curr_in, v_in_dc_nyq_stride, v_in0, is_contiguous_in_dc_nyq);
        // Input point 2: x(1) & Input point 3: x(2)
        curr_in = in_cp + in_strides[1];
        LDRI_2x512_S(curr_in, v_in_stride, v_in1, v_in2);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_512_S(curr_in, v_in_dc_nyq_stride, v_in3, is_contiguous_in_dc_nyq);

        v_s0 = _mm512_add_ps(v_in0, v_in3);
        v_t0 = _mm512_mul_ps(v_CRTM_4_1, v_in1);
        v_s1 = _mm512_sub_ps(v_in0, v_in3);
        v_t1 = _mm512_mul_ps(v_CRTM_4_1, v_in2);

        // Output point 1: X(0)
        v_out0 = _mm512_add_ps(v_s0, v_t0);

        // Output point 2: X(1)
        v_out1 = _mm512_sub_ps(v_s1, v_t1);

        // Output point 3: X(2)
        v_out2 = _mm512_sub_ps(v_s0, v_t0);

        // Output point 4: X(3)
        v_out3 = _mm512_add_ps(v_s1, v_t1);

        STR_512_S(curr_out, v_out_stride, v_out0, is_contiguous_out);
        curr_out = out_r + out_strides[1];
        STR_512_S(curr_out, v_out_stride, v_out1, is_contiguous_out);
        curr_out = out_r + out_strides[2];
        STR_512_S(curr_out, v_out_stride, v_out2, is_contiguous_out);
        curr_out = out_r + out_strides[3];
        STR_512_S(curr_out, v_out_stride, v_out3, is_contiguous_out);

        in_r = in_r + (v_in_dc_nyq_stride << 4);
        in_cp  = in_cp + (v_in_stride << 4);
        out_r = out_r + (v_out_stride << 4);
    }
    // tail cases
    if (remaining_sets & NUM_SETS_REAL_256_S)
    {
        __m256 v_in0, v_in1, v_in2, v_in3;
        __m256 v_s0, v_s1;
        __m256 v_t0, v_t1;
        __m256 v_out0, v_out1, v_out2, v_out3;

        __m256 v256_CRTM_4_1 = _mm512_castps512_ps256(v_CRTM_4_1);

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_256_S(curr_in, v_in_dc_nyq_stride, v_in0, is_contiguous_in_dc_nyq);
        // Input point 2: x(1) & Input point 3: x(2)
        curr_in = in_cp + in_strides[1];
        LDRI_2x256_S(curr_in, v_in_stride, v_in1, v_in2);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_256_S(curr_in, v_in_dc_nyq_stride, v_in3, is_contiguous_in_dc_nyq);

        v_s0 = _mm256_add_ps(v_in0, v_in3);
        v_t0 = _mm256_mul_ps(v256_CRTM_4_1, v_in1);
        v_s1 = _mm256_sub_ps(v_in0, v_in3);
        v_t1 = _mm256_mul_ps(v256_CRTM_4_1, v_in2);

        // Output point 1: X(0)
        v_out0 = _mm256_add_ps(v_s0, v_t0);

        // Output point 2: X(1)
        v_out1 = _mm256_sub_ps(v_s1, v_t1);

        // Output point 3: X(2)
        v_out2 = _mm256_sub_ps(v_s0, v_t0);

        // Output point 4: X(3)
        v_out3 = _mm256_add_ps(v_s1, v_t1);

        STR_256_S(curr_out, v_out_stride, v_out0, is_contiguous_out);
        curr_out = out_r + out_strides[1];
        STR_256_S(curr_out, v_out_stride, v_out1, is_contiguous_out);
        curr_out = out_r + out_strides[2];
        STR_256_S(curr_out, v_out_stride, v_out2, is_contiguous_out);
        curr_out = out_r + out_strides[3];
        STR_256_S(curr_out, v_out_stride, v_out3, is_contiguous_out);

        in_r = in_r + (v_in_dc_nyq_stride << 3);
        in_cp  = in_cp + (v_in_stride << 3);
        out_r = out_r + (v_out_stride << 3);
    }
    // tail cases
    if (remaining_sets & NUM_SETS_REAL_128_S)
    {
        __m128 v_in0, v_in1, v_in2, v_in3;
        __m128 v_s0, v_s1;
        __m128 v_t0, v_t1;
        __m128 v_out0, v_out1, v_out2, v_out3;

        __m128 v128_CRTM_4_1 = _mm512_castps512_ps128(v_CRTM_4_1);

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_128_S(curr_in, v_in_dc_nyq_stride, v_in0, is_contiguous_in_dc_nyq);
        // Input point 2: x(1) & Input point 3: x(2)
        curr_in = in_cp + in_strides[1];
        LDRI_2x128_S(curr_in, v_in_stride, v_in1, v_in2);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_128_S(curr_in, v_in_dc_nyq_stride, v_in3, is_contiguous_in_dc_nyq);

        v_s0 = _mm_add_ps(v_in0, v_in3);
        v_t0 = _mm_mul_ps(v128_CRTM_4_1, v_in1);
        v_s1 = _mm_sub_ps(v_in0, v_in3);
        v_t1 = _mm_mul_ps(v128_CRTM_4_1, v_in2);

        // Output point 1: X(0)
        v_out0 = _mm_add_ps(v_s0, v_t0);

        // Output point 2: X(1)
        v_out1 = _mm_sub_ps(v_s1, v_t1);

        // Output point 3: X(2)
        v_out2 = _mm_sub_ps(v_s0, v_t0);

        // Output point 4: X(3)
        v_out3 = _mm_add_ps(v_s1, v_t1);

        STR_128_S(curr_out, v_out_stride, v_out0, is_contiguous_out);
        curr_out = out_r + out_strides[1];
        STR_128_S(curr_out, v_out_stride, v_out1, is_contiguous_out);
        curr_out = out_r + out_strides[2];
        STR_128_S(curr_out, v_out_stride, v_out2, is_contiguous_out);
        curr_out = out_r + out_strides[3];
        STR_128_S(curr_out, v_out_stride, v_out3, is_contiguous_out);

        in_r = in_r + (v_in_dc_nyq_stride << 2);
        in_cp  = in_cp + (v_in_stride << 2);
        out_r = out_r + (v_out_stride << 2);
    }
    // tail cases
    if (remaining_sets & 2)
    {
        __m128 v_in0, v_in1, v_in2, v_in3;
        __m128 v_s0, v_s1;
        __m128 v_t0, v_t1;
        __m128 v_out0, v_out1, v_out2, v_out3;

        __m128 v128_CRTM_4_1 = _mm512_castps512_ps128(v_CRTM_4_1);

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDHR_128_S(curr_in, v_in_dc_nyq_stride, v_in0);
        // Input point 2: x(1) & Input point 3: x(2)
        curr_in = in_cp + in_strides[1];
        LDHRI_2x128_S(curr_in, v_in_stride, v_in1, v_in2);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDHR_128_S(curr_in, v_in_dc_nyq_stride, v_in3);

        v_s0 = _mm_add_ps(v_in0, v_in3);
        v_t0 = _mm_mul_ps(v128_CRTM_4_1, v_in1);
        v_s1 = _mm_sub_ps(v_in0, v_in3);
        v_t1 = _mm_mul_ps(v128_CRTM_4_1, v_in2);

        // Output point 1: X(0)
        v_out0 = _mm_add_ps(v_s0, v_t0);

        // Output point 2: X(1)
        v_out1 = _mm_sub_ps(v_s1, v_t1);

        // Output point 3: X(2)
        v_out2 = _mm_sub_ps(v_s0, v_t0);

        // Output point 4: X(3)
        v_out3 = _mm_add_ps(v_s1, v_t1);

        STHR_128_S(curr_out, v_out_stride, v_out0);
        curr_out = out_r + out_strides[1];
        STHR_128_S(curr_out, v_out_stride, v_out1);
        curr_out = out_r + out_strides[2];
        STHR_128_S(curr_out, v_out_stride, v_out2);
        curr_out = out_r + out_strides[3];
        STHR_128_S(curr_out, v_out_stride, v_out3);

        in_r = in_r + (v_in_dc_nyq_stride << 1);
        in_cp  = in_cp + (v_in_stride << 1);
        out_r = out_r + (v_out_stride << 1);
    }
    // tail cases
    if (remaining_sets & 1)
    {
        FFTZ_FLOAT in0, in1, in2, in3;
        FFTZ_FLOAT s0, s1;
        FFTZ_FLOAT t0, t1;

        curr_in = in_r;
        curr_out = out_r;
        // Input point 1: x(0)
        in0 = *in_r;
        // Input point 2: x(1)
        in1 = in_cp[in_strides[1]];
        // Input point 3: x(2)
        in2 = in_cp[in_strides[2]];
        // Input point 4: x(3)
        in3 = in_r[in_strides[3]];

        s0 = in0 + in3;
        t0 = CRTM_4_1 * in1;
        s1 = in0 - in3;
        t1 = CRTM_4_1 * in2;

        // Output point 1: X(0)
        *out_r = s0 + t0;

        // Output point 2: X(1)
        out_r[out_strides[1]] = s1 - t1;

        // Output point 3: X(2)
        out_r[out_strides[2]] = s0 - t0;

        // Output point 4: X(3)
        out_r[out_strides[3]] = s1 + t1;
    }
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Exit");
}

static FFTZ_VOID r2hc_rfft4avx512_fp64_fwd(FFTZ_VOID *in_real,
                                           FFTZ_VOID *in_complex,
                                           FFTZ_VOID *out_real,
                                           FFTZ_VOID *out_complex, FFTZ_INTP n,
                                           aoclfftz_strides_t *strides,
                                           FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Enter");

    FFTZ_DOUBLE *in_r = (FFTZ_DOUBLE *)in_real;
    FFTZ_DOUBLE *out_r = (FFTZ_DOUBLE *)out_real;
    FFTZ_DOUBLE *out_cp = (FFTZ_DOUBLE *)out_complex;
#ifdef VOLATILE_STRIDE_ARRAY
    volatile FFTZ_INTP *in_strides = strides->in_strides;
    volatile FFTZ_INTP *out_strides = strides->out_strides;
#else
    FFTZ_INTP *in_strides  = strides->in_strides;
    FFTZ_INTP *out_strides = strides->out_strides;
#endif
    FFTZ_INTP v_in_stride  = strides->v_in_stride;
    FFTZ_INTP v_out_stride = strides->v_out_stride;
    FFTZ_UINT8 is_contiguous_in = (v_in_stride == 1);
    // Used for endpoints: DC and Nyquist.
    FFTZ_INTP v_out_dc_nyq_stride = strides->v_out_sym_stride;
    FFTZ_UINT8 is_contiguous_out_dc_nyq = (v_out_dc_nyq_stride == 1);

    FFTZ_INTP cnt;
    FFTZ_DOUBLE *curr_in, *curr_out;
    FFTZ_INTP N = n / NUM_SETS_REAL_512_D;
    FFTZ_INTP remaining_sets = n % NUM_SETS_REAL_512_D;

    for (cnt = 0; cnt < N; cnt++)
    {
        __m512d v_in0, v_in1, v_in2, v_in3;
        __m512d v_s0, v_s1;
        __m512d v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_512_D(curr_in, v_in_stride, v_in0, is_contiguous_in);
        // Input point 2: x(1)
        curr_in = in_r + in_strides[1];
        LDR_512_D(curr_in, v_in_stride, v_in1, is_contiguous_in);
        // Input point 3: x(2)
        curr_in = in_r + in_strides[2];
        LDR_512_D(curr_in, v_in_stride, v_in2, is_contiguous_in);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_512_D(curr_in, v_in_stride, v_in3, is_contiguous_in);

        v_s0 = _mm512_add_pd(v_in0, v_in2);
        v_s1 = _mm512_add_pd(v_in1, v_in3);

        // Output point 1: X(0)
        v_out0 = _mm512_add_pd(v_s0, v_s1);

        // Output point 2: X(1)
        v_out1 = _mm512_sub_pd(v_in0, v_in2);

        // Output point 3: X(2)
        v_out2 = _mm512_sub_pd(v_in3, v_in1);

        // Output point 4: X(3)
        v_out3 = _mm512_sub_pd(v_s0, v_s1);
        STR_512_D(curr_out, v_out_dc_nyq_stride, v_out0, is_contiguous_out_dc_nyq);
        curr_out = out_cp + out_strides[1];
        STRI_2x512_D(curr_out, v_out_stride, v_out1, v_out2);
        curr_out = out_r + out_strides[3];
        STR_512_D(curr_out, v_out_dc_nyq_stride, v_out3, is_contiguous_out_dc_nyq);

        in_r  = in_r + (v_in_stride << 3);
        out_cp = out_cp + (v_out_stride << 3);
        out_r = out_r + (v_out_dc_nyq_stride << 3);
    }
    // tail cases
    if (remaining_sets & NUM_SETS_REAL_256_D)
    {
        __m256d v_in0, v_in1, v_in2, v_in3;
        __m256d v_s0, v_s1;
        __m256d v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_256_D(curr_in, v_in_stride, v_in0, is_contiguous_in);
        // Input point 2: x(1)
        curr_in = in_r + in_strides[1];
        LDR_256_D(curr_in, v_in_stride, v_in1, is_contiguous_in);
        // Input point 3: x(2)
        curr_in = in_r + in_strides[2];
        LDR_256_D(curr_in, v_in_stride, v_in2, is_contiguous_in);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_256_D(curr_in, v_in_stride, v_in3, is_contiguous_in);

        v_s0 = _mm256_add_pd(v_in0, v_in2);
        v_s1 = _mm256_add_pd(v_in1, v_in3);

        // Output point 1: X(0)
        v_out0 = _mm256_add_pd(v_s0, v_s1);

        // Output point 2: X(1)
        v_out1 = _mm256_sub_pd(v_in0, v_in2);

        // Output point 3: X(2)
        v_out2 = _mm256_sub_pd(v_in3, v_in1);

        // Output point 4: X(3)
        v_out3 = _mm256_sub_pd(v_s0, v_s1);
        STR_256_D(curr_out, v_out_dc_nyq_stride, v_out0, is_contiguous_out_dc_nyq);
        curr_out = out_cp + out_strides[1];
        STRI_2x256_D(curr_out, v_out_stride, v_out1, v_out2);
        curr_out = out_r + out_strides[3];
        STR_256_D(curr_out, v_out_dc_nyq_stride, v_out3, is_contiguous_out_dc_nyq);

        in_r  = in_r + (v_in_stride << 2);
        out_cp = out_cp + (v_out_stride << 2);
        out_r = out_r + (v_out_dc_nyq_stride << 2);
    }
    // tail cases
    if (remaining_sets & NUM_SETS_REAL_128_D)
    {
        __m128d v_in0, v_in1, v_in2, v_in3;
        __m128d v_s0, v_s1;
        __m128d v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_128_D(curr_in, v_in_stride, v_in0, is_contiguous_in);
        // Input point 2: x(1)
        curr_in = in_r + in_strides[1];
        LDR_128_D(curr_in, v_in_stride, v_in1, is_contiguous_in);
        // Input point 3: x(2)
        curr_in = in_r + in_strides[2];
        LDR_128_D(curr_in, v_in_stride, v_in2, is_contiguous_in);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_128_D(curr_in, v_in_stride, v_in3, is_contiguous_in);

        v_s0 = _mm_add_pd(v_in0, v_in2);
        v_s1 = _mm_add_pd(v_in1, v_in3);

        // Output point 1: X(0)
        v_out0 = _mm_add_pd(v_s0, v_s1);

        // Output point 2: X(1)
        v_out1 = _mm_sub_pd(v_in0, v_in2);

        // Output point 3: X(2)
        v_out2 = _mm_sub_pd(v_in3, v_in1);

        // Output point 4: X(3)
        v_out3 = _mm_sub_pd(v_s0, v_s1);
        STR_128_D(curr_out, v_out_dc_nyq_stride, v_out0, is_contiguous_out_dc_nyq);
        curr_out = out_cp + out_strides[1];
        STRI_2x128_D(curr_out, v_out_stride, v_out1, v_out2);
        curr_out = out_r + out_strides[3];
        STR_128_D(curr_out, v_out_dc_nyq_stride, v_out3, is_contiguous_out_dc_nyq);

        in_r  = in_r + (v_in_stride << 1);
        out_cp = out_cp + (v_out_stride << 1);
        out_r = out_r + (v_out_dc_nyq_stride << 1);
    }
    // tail cases
    if (remaining_sets & 1)
    {
        FFTZ_DOUBLE in0, in1, in2, in3;
        FFTZ_DOUBLE s0, s1;

        curr_in = in_r;
        curr_out = out_r;
        // Input point 1: x(0)
        in0 = *in_r;
        // Input point 2: x(1)
        in1 = in_r[in_strides[1]];
        // Input point 3: x(2)
        in2 = in_r[in_strides[2]];
        // Input point 4: x(3)
        in3 = in_r[in_strides[3]];

        s0 = in0 + in2;
        s1 = in1 + in3;

        // Output point 1: X(0)
        *out_r = s0 + s1;

        // Output point 2: X(1)
        out_cp[out_strides[1]] = in0 - in2;

        // Output point 3: X(2)
        out_cp[out_strides[2]] = in3 - in1;

        // Output point 4: X(3)
        out_r[out_strides[3]] = s0 - s1;
    }
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Exit");
}

static FFTZ_VOID r2hc_rfft4avx512_fp64_bwd(FFTZ_VOID *in_real,
                                           FFTZ_VOID *in_complex,
                                           FFTZ_VOID *out_real,
                                           FFTZ_VOID *out_complex, FFTZ_INTP n,
                                           aoclfftz_strides_t *strides,
                                           FFTZ_VOID *twd, FFTZ_UINT8 flag)
{
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Enter");
    const FFTZ_DOUBLE CRTM_4_1 =
        2.000000000000000000000000000000000000000000000;

    FFTZ_DOUBLE *in_r = (FFTZ_DOUBLE *)in_real;
    FFTZ_DOUBLE *in_cp = (FFTZ_DOUBLE *)in_complex;
    FFTZ_DOUBLE *out_r = (FFTZ_DOUBLE *)out_real;
#ifdef VOLATILE_STRIDE_ARRAY
    volatile FFTZ_INTP *in_strides = strides->in_strides;
    volatile FFTZ_INTP *out_strides = strides->out_strides;
#else
    FFTZ_INTP *in_strides  = strides->in_strides;
    FFTZ_INTP *out_strides = strides->out_strides;
#endif
    FFTZ_INTP v_in_stride  = strides->v_in_stride;
    FFTZ_INTP v_out_stride = strides->v_out_stride;
    FFTZ_UINT8 is_contiguous_out = (v_out_stride == 1);
    // Used for endpoints: DC and Nyquist.
    FFTZ_INTP v_in_dc_nyq_stride = strides->v_in_sym_stride;
    FFTZ_UINT8 is_contiguous_in_dc_nyq = (v_in_dc_nyq_stride == 1);

    FFTZ_INTP cnt;
    FFTZ_DOUBLE *curr_in, *curr_out;
    FFTZ_INTP N = n / NUM_SETS_REAL_512_D;
    FFTZ_INTP remaining_sets = n % NUM_SETS_REAL_512_D;

    __m512d v_CRTM_4_1 = _mm512_set1_pd(CRTM_4_1);

    for (cnt = 0; cnt < N; cnt++)
    {
        __m512d v_in0, v_in1, v_in2, v_in3;
        __m512d v_s0, v_s1;
        __m512d v_t0, v_t1;
        __m512d v_out0, v_out1, v_out2, v_out3;

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_512_D(curr_in, v_in_dc_nyq_stride, v_in0, is_contiguous_in_dc_nyq);
        // Input point 2: x(1) & Input point 3: x(2)
        curr_in = in_cp + in_strides[1];
        LDRI_2x512_D(curr_in, v_in_stride, v_in1, v_in2);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_512_D(curr_in, v_in_dc_nyq_stride, v_in3, is_contiguous_in_dc_nyq);

        v_s0 = _mm512_add_pd(v_in0, v_in3);
        v_t0 = _mm512_mul_pd(v_CRTM_4_1, v_in1);
        v_s1 = _mm512_sub_pd(v_in0, v_in3);
        v_t1 = _mm512_mul_pd(v_CRTM_4_1, v_in2);

        // Output point 1: X(0)
        v_out0 = _mm512_add_pd(v_s0, v_t0);

        // Output point 2: X(1)
        v_out1 = _mm512_sub_pd(v_s1, v_t1);

        // Output point 3: X(2)
        v_out2 = _mm512_sub_pd(v_s0, v_t0);

        // Output point 4: X(3)
        v_out3 = _mm512_add_pd(v_s1, v_t1);

        STR_512_D(curr_out, v_out_stride, v_out0, is_contiguous_out);
        curr_out = out_r + out_strides[1];
        STR_512_D(curr_out, v_out_stride, v_out1, is_contiguous_out);
        curr_out = out_r + out_strides[2];
        STR_512_D(curr_out, v_out_stride, v_out2, is_contiguous_out);
        curr_out = out_r + out_strides[3];
        STR_512_D(curr_out, v_out_stride, v_out3, is_contiguous_out);

        in_r = in_r + (v_in_dc_nyq_stride << 3);
        in_cp  = in_cp + (v_in_stride << 3);
        out_r = out_r + (v_out_stride << 3);
    }
    // tail cases
    if (remaining_sets & NUM_SETS_REAL_256_D)
    {
        __m256d v_in0, v_in1, v_in2, v_in3;
        __m256d v_s0, v_s1;
        __m256d v_t0, v_t1;
        __m256d v_out0, v_out1, v_out2, v_out3;

        __m256d v256_CRTM_4_1 = _mm512_castpd512_pd256(v_CRTM_4_1);

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_256_D(curr_in, v_in_dc_nyq_stride, v_in0, is_contiguous_in_dc_nyq);
        // Input point 2: x(1) & Input point 3: x(2)
        curr_in = in_cp + in_strides[1];
        LDRI_2x256_D(curr_in, v_in_stride, v_in1, v_in2);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_256_D(curr_in, v_in_dc_nyq_stride, v_in3, is_contiguous_in_dc_nyq);

        v_s0 = _mm256_add_pd(v_in0, v_in3);
        v_t0 = _mm256_mul_pd(v256_CRTM_4_1, v_in1);
        v_s1 = _mm256_sub_pd(v_in0, v_in3);
        v_t1 = _mm256_mul_pd(v256_CRTM_4_1, v_in2);

        // Output point 1: X(0)
        v_out0 = _mm256_add_pd(v_s0, v_t0);

        // Output point 2: X(1)
        v_out1 = _mm256_sub_pd(v_s1, v_t1);

        // Output point 3: X(2)
        v_out2 = _mm256_sub_pd(v_s0, v_t0);

        // Output point 4: X(3)
        v_out3 = _mm256_add_pd(v_s1, v_t1);

        STR_256_D(curr_out, v_out_stride, v_out0, is_contiguous_out);
        curr_out = out_r + out_strides[1];
        STR_256_D(curr_out, v_out_stride, v_out1, is_contiguous_out);
        curr_out = out_r + out_strides[2];
        STR_256_D(curr_out, v_out_stride, v_out2, is_contiguous_out);
        curr_out = out_r + out_strides[3];
        STR_256_D(curr_out, v_out_stride, v_out3, is_contiguous_out);

        in_r = in_r + (v_in_dc_nyq_stride << 2);
        in_cp  = in_cp + (v_in_stride << 2);
        out_r = out_r + (v_out_stride << 2);
    }
    // tail cases
    if (remaining_sets & NUM_SETS_REAL_128_D)
    {
        __m128d v_in0, v_in1, v_in2, v_in3;
        __m128d v_s0, v_s1;
        __m128d v_t0, v_t1;
        __m128d v_out0, v_out1, v_out2, v_out3;

        __m128d v128_CRTM_4_1 = _mm512_castpd512_pd128(v_CRTM_4_1);

        curr_in = in_r;
        curr_out = out_r;

        // Input point 1: x(0)
        LDR_128_D(curr_in, v_in_dc_nyq_stride, v_in0, is_contiguous_in_dc_nyq);
        // Input point 2: x(1) & Input point 3: x(2)
        curr_in = in_cp + in_strides[1];
        LDRI_2x128_D(curr_in, v_in_stride, v_in1, v_in2);
        // Input point 4: x(3)
        curr_in = in_r + in_strides[3];
        LDR_128_D(curr_in, v_in_dc_nyq_stride, v_in3, is_contiguous_in_dc_nyq);

        v_s0 = _mm_add_pd(v_in0, v_in3);
        v_t0 = _mm_mul_pd(v128_CRTM_4_1, v_in1);
        v_s1 = _mm_sub_pd(v_in0, v_in3);
        v_t1 = _mm_mul_pd(v128_CRTM_4_1, v_in2);

        // Output point 1: X(0)
        v_out0 = _mm_add_pd(v_s0, v_t0);

        // Output point 2: X(1)
        v_out1 = _mm_sub_pd(v_s1, v_t1);

        // Output point 3: X(2)
        v_out2 = _mm_sub_pd(v_s0, v_t0);

        // Output point 4: X(3)
        v_out3 = _mm_add_pd(v_s1, v_t1);

        STR_128_D(curr_out, v_out_stride, v_out0, is_contiguous_out);
        curr_out = out_r + out_strides[1];
        STR_128_D(curr_out, v_out_stride, v_out1, is_contiguous_out);
        curr_out = out_r + out_strides[2];
        STR_128_D(curr_out, v_out_stride, v_out2, is_contiguous_out);
        curr_out = out_r + out_strides[3];
        STR_128_D(curr_out, v_out_stride, v_out3, is_contiguous_out);

        in_r = in_r + (v_in_dc_nyq_stride << 1);
        in_cp  = in_cp + (v_in_stride << 1);
        out_r = out_r + (v_out_stride << 1);
    }
    // tail cases
    if (remaining_sets & 1)
    {
        FFTZ_DOUBLE in0, in1, in2, in3;
        FFTZ_DOUBLE s0, s1;
        FFTZ_DOUBLE t0, t1;

        curr_in = in_r;
        curr_out = out_r;
        // Input point 1: x(0)
        in0 = *in_r;
        // Input point 2: x(1)
        in1 = in_cp[in_strides[1]];
        // Input point 3: x(2)
        in2 = in_cp[in_strides[2]];
        // Input point 4: x(3)
        in3 = in_r[in_strides[3]];

        s0 = in0 + in3;
        t0 = CRTM_4_1 * in1;
        s1 = in0 - in3;
        t1 = CRTM_4_1 * in2;

        // Output point 1: X(0)
        *out_r = s0 + t0;

        // Output point 2: X(1)
        out_r[out_strides[1]] = s1 - t1;

        // Output point 3: X(2)
        out_r[out_strides[2]] = s0 - t0;

        // Output point 4: X(3)
        out_r[out_strides[3]] = s1 + t1;
    }
    AOCLFFTZ_LOG(DEBUG, global_logger_mode, "Exit");
}

kfft_ register_kernel_r2hc_rfft4avx512(FFTZ_UINT8 precision,
                                       FFTZ_UINT8 direction)
{
    if (direction == FORWARD_FFT_DIR)
    {
        if (precision == DT_FLOAT)
        {
            return r2hc_rfft4avx512_fp32_fwd;
        }
        else if (precision == DT_DOUBLE)
        {
            return r2hc_rfft4avx512_fp64_fwd;
        }
        else
        {
            return NULL;
        }
    }
    else
    {
        if (precision == DT_FLOAT)
        {
            return r2hc_rfft4avx512_fp32_bwd;
        }
        else if (precision == DT_DOUBLE)
        {
            return r2hc_rfft4avx512_fp64_bwd;
        }
        else
        {
            return NULL;
        }
    }
}
